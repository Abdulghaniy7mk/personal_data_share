#!/bin/bash
# ╔══════════════════════════════════════════════════════════════╗
# ║              CYBERLAB - Hacking Tools Launcher               ║
# ║         Rofi-based tool browser for Arch Linux               ║
# ╚══════════════════════════════════════════════════════════════╝
# Run: bash cyberlab-setup.sh
...
SELECTION=$(echo "$MENU" | rofi \
    ...
    -i \
    -p "⚡ cyberlab" \
    -mesg "  ${INSTALLED} tools installed  │  blackarch has ${TOTAL}+ available  │  select to view usage" \
    -theme "$THEME" \
    -no-custom \
    -format s \
    -markup-rows \
...
echo ""
echo -e "  ${GREEN}HYPRLAND KEYBIND${NC} — add to ~/.config/hypr/hyprland.conf:"
echo -e "  ${DIM}bind = SUPER, F1, exec, cyberlab${NC}"
echo ""
echo -e "  ${GREEN}INSTALL TOOLS (BlackArch repo):${NC}"
echo -e "  ${DIM}# Add BlackArch to pacman (one-time):${NC}"
echo -e "  ${DIM}curl -O https://blackarch.org/strap.sh${NC}"
echo -e "  ${DIM}chmod +x strap.sh && sudo bash strap.sh${NC}"
echo ""
echo -e "  ${DIM}# Then install any tool:${NC}"
echo -e "  ${DIM}sudo pacman -S nmap sqlmap hashcat hydra aircrack-ng${NC}"
echo -e "  ${DIM}sudo pacman -S blackarch          # install EVERYTHING (~2600 tools)${NC}"
echo ""
